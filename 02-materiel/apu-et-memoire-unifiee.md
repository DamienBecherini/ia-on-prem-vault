...
> 🔗 **Lien connexe :** pour le dimensionnement mémoire (poids + KV cache), voir [[01-fondations/quantification-4-bit-8-bit|Quantification]] et [[01-fondations/kv-cache-et-contexte|KV Cache]].
...
> Les vitesses Apple proviennent de benchmarks communautaires **MLX** ; **llama.cpp/Metal** est souvent légèrement plus lent en decode pur, mais plus flexible en long contexte [^9][^10]. La borne théorique memory-bound (~13–21 tok/s Apple vs ~7 tok/s AMD) est détaillée dans [[01-fondations/memoire-unifiee-vs-ram-vs-vram|Mémoire unifiée vs RAM vs VRAM]].
