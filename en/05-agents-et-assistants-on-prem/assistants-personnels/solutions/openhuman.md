...
OpenHuman is an open-source personal assistant based on **Tauri + Rust**. Its core idea is the **[[00-lexique/memory-tree|Memory Tree]]**: your sources are turned into Markdown chunks, summarized hierarchically, stored in SQLite, and exposed as an Obsidian-compatible vault[^1][^2].

It is not a 100% on-premise tool by default. The README is explicit: the managed experience still uses OpenHuman services for account login, model routing, proxied web search, and some OAuth flows via Composio[^3].
...
OpenHuman is the best current example of a "memory-first" assistant: it does not just slap a vector database under a chat. It structures documents into summary trees and keeps a human-readable Markdown equivalent.
...
- **Local AI possible:** Ollama/LM Studio can handle some workloads on-device[^3].

## ⚠️ Limitations and risks

- **Cloud by default for several critical functions:** LLM routing, proxied web search, managed OAuth/integrations[^3].
- **Non-trivial sovereignty:** managed paths must be replaced one by one.
- **Young project:** interesting but audit before sensitive enterprise use.
...
- **Telemetry:** verify in deployed instance.
...
- a comparison sheet to explain the "local-first ≠ sovereign by default" trap;
- an example of a hybrid solution not to present as strict on-prem without caveats.

## 📊 Project maturity

Rapidly evolving open-source project. Audit before client deployment: release frequency, network dependencies, authentication model, OAuth token retention policy, and real self-host options.
